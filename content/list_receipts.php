...
,m.mem_status,
p.person_id as id,p.person_id as id2
FROM person p
...
$column=array("รหัสสมาชิก","ชื่อ-นามสกุล","ประเภทสามชิก","สถานะการเป็นสมาชิก","รายละเอียด","รับเงิน");//หากเป็น TB_mng ต้องเพิ่ม แก้ไข,ลบเข้าไปด้วย 
                $mydata=new TablePDO($column);
                $read="connection/conn_DB.txt";
                $mydata->para_read($read);
                $db=$mydata->conn_PDO();
                $mydata->imp_sql($sql);
                $mydata->createPDO_TB_ED('receipts');//ใส่ process ที่ต้องการสร้าง
                $mydata->close_PDO();
    ?>
                </div>
              </div>
